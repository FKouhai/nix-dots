...
let
  ...
  isFeatureDir =
    name: type: type == "directory" && builtins.pathExists (featuresDir + "/${name}/default.nix");
  ...
  getSubFeatures =
    ...
    in
    lib.mapAttrs (name: _: import (parentDir + "/${name}")) subFeatureDirs;

  mkFeature = name: (import (featuresDir + "/${name}")) // (getSubFeatures name);
in
{
